...
export function AdminDashboard() {
  const [data, setData] = useState<{ products: Product[]; totalOrders: number } | null>(null)
  const [error, setError] = useState('')
  useEffect(() => {
    let active = true
    Promise.all([api.products(true), api.orders({ page: 1, pageSize: 1 })])
      .then(([products, orders]) => { if (active) setData({ products, totalOrders: orders.totalItems }) })
  ...
  return <section className="admin-page">
    ...
    <p className="admin-footnote">Stock bajo se considera entre 1 y {LOW_STOCK_THRESHOLD} unidades.</p>
  </section>
}
