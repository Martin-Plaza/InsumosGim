...
export function ProductEditorPage({ mode }: { mode: 'create' | 'edit' }) {
  ...
  const [loading, setLoading] = useState(true); const [loadError, setLoadError] = useState(''); const [notFound, setNotFound] = useState(false); const [retryKey, setRetryKey] = useState(0); const [submitError, setSubmitError] = useState(''); const [fieldErrors, setFieldErrors] = useState<ProductFormErrors>({}); const [busy, setBusy] = useState(false)
  ...
  const save = async (values: ProductFormValues) => {
    ...
    let completed = false
    try {
      const saved = mode === 'create' ? await api.createProduct(toProductInput(values)) : await api.updateProduct(numericId, toUpdateProductInput(values))
      completed = true
      ...
      if (!completed) { submitting.current = false; setBusy(false) }
    }
  }
