  ...
  return <form className="product-form" noValidate onSubmit={event => {
    ...
    <div className="product-form-fields">
      ...
      <div className="product-form-row">
        ...
        <label>Stock<input aria-label="Stock" name="stock" type="number" min="0" step="1" value={values.stock} disabled={mode === 'edit'} aria-invalid={Boolean(errors.stock)} aria-describedby={errors.stock ? 'stock-error' : mode === 'edit' ? 'stock-managed-help' : undefined} onChange={event => change('stock', event.target.value)} />{fieldError('stock')}{mode === 'edit' && <small id="stock-managed-help">Los cambios de stock se realizan desde la sección Stock para conservar su historial.</small>}</label>
      </div>
